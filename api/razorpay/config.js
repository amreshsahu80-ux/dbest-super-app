...
async function capturedPaymentForOrder(order,keyId,secret){const ps=await rzFetch('/orders/'+encodeURIComponent(order.id)+'/payments',keyId,secret),items=Array.isArray(ps?.items)?ps.items:[];let payment=items.find(x=>x&&x.status==='captured')||items.find(x=>x&&x.status==='authorized')||null;if(!payment)return null;if(payment.status==='authorized')payment=await rzFetch('/payments/'+encodeURIComponent(payment.id)+'/capture',keyId,secret,{method:'POST',body:JSON.stringify({amount:Number(order.amount),currency:String(order.currency||'INR')})});return payment&&payment.status==='captured'?payment:null;}
async function recoverServicePartner(b,keyId,secret,persistenceKey){
  const orderId=safe(b.orderId||b.razorpay_order_id,120),dbestRef=safe(b.dbestRef,100),userId=safe(b.userId||b.email,180).toLowerCase();
  if(!orderId||!dbestRef)return {status:400,body:{error:'Recovery reference is incomplete',verified:false}};
  ...
  if(String(notes.kind||'').toLowerCase()!=='service_partner_registration')return {status:403,body:{error:'Payment purpose mismatch',verified:false}};
  if(userId&&String(notes.user_id||'').toLowerCase()!==userId)return {status:403,body:{error:'Service Partner identity mismatch',verified:false}};
  if(Number(order.amount)!==9900)return {status:400,body:{error:'Service Partner fee mismatch',verified:false}};
  const payment=await capturedPaymentForOrder(order,keyId,secret);
  if(!payment)return {status:409,body:{error:'Payment not captured yet',verified:false,status:'pending'}};
  if(Number(payment.amount||0)!==9900)return {status:400,body:{error:'Payment amount mismatch',verified:false}};
  const now=new Date().toISOString();
  const tx={transaction_id:dbestRef,transaction_date:now,section:'Service Partner Onboarding',subsection:'Annual Onboarding Fee',actor_type:'Service Partner',actor_name:safe(payment.email||userId||payment.contact||'Service Partner',140),actor_ref:safe(userId||payment.email||payment.contact,180),counterparty_type:'Company',counterparty_name:'Sarwashresth Services OPC Pvt. Ltd.',amount:99,payment_mode:'Razorpay',payment_status:'Verified',reference:String(payment.id||''),payout_amount:0,metadata:{dbest_ref:dbestRef,kind:'service_partner_registration',section:'Service Partner Onboarding',subsection:'Annual Onboarding Fee',user_id:String(notes.user_id||userId||''),razorpay_order_id:String(order.id||''),razorpay_payment_id:String(payment.id||''),razorpay_method:String(payment.method||''),currency:String(payment.currency||order.currency||'INR'),email:String(payment.email||''),contact:String(payment.contact||''),source:'DBest Super Platform',environment:process.env.VERCEL_ENV||'preview',verified_at:now,recovered:true},updated_at:now};
  ...
  return {status:200,body:{verified:true,recovered:true,persisted:true,paymentId:String(payment.id||''),orderId:String(order.id||''),amount:Number(payment.amount||0),status:String(payment.status||''),method:String(payment.method||'')}};
}
async function recoverMarketplace(b,keyId,secret,persistenceKey){
  ...
  const expected=Math.round(Number(m.total_amount||0)*100);if(expected!==Number(payment.amount||0)||expected!==Number(order.amount||0))return {status:400,body:{error:'Marketplace total does not match Razorpay amount',verified:false}};
  const now=new Date().toISOString();
...
module.exports=async function handler(req,res){
  ...
  const keyId=String(process.env.RAZORPAY_KEY_ID||'').trim(),secret=String(process.env.RAZORPAY_KEY_SECRET||'').trim(),persistenceKey=serverKey();
  if(req.method==='GET'){
    const stage=String((req.query&&req.query.stage)||'').replace(/[^a-zA-Z0-9_-]/g,'').slice(0,60);if(process.env.VERCEL_ENV==='preview'&&stage)console.log('[DBEST_RZP_TRACE]',stage);
  ...
  if(req.method==='POST'){
    const b=req.body&&typeof req.body==='object'?req.body:{},action=String(b.action||'');
    if(!['recover-marketplace','recover-service-partner'].includes(action))return res.status(400).json({error:'Unsupported action'});
