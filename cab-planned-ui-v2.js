...
const F={p:null,d:null,stops:[],stopTexts:[],route:null,selected:'auto',mode:'ride',tripType:'oneway',nightHalt:false,rentalHours:2,rentalKm:20,schedule:'now',scheduledAt:'',rider:'self',riderName:'',riderMobile:'',map:null,calculating:false};
...
.dbestTripType{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin:8px 0 10px}.dbestTripType button{min-height:44px;border:1px solid #dfe4ef;border-radius:14px;background:#fff;color:#344566;font-size:11px;font-weight:900}.dbestTripType button.on{background:linear-gradient(135deg,#1f64f3,#674de9);color:#fff;border-color:transparent;box-shadow:0 8px 18px rgba(54,77,205,.2)}
#dbestRoundTripNote{display:none;margin:6px 0 10px;padding:10px 11px;border-radius:14px;background:#f4f7ff;border:1px solid #dfe7fb;color:#43506d;font-size:9.5px;line-height:1.45}#dbestRoundTripNote.show{display:block}#dbestRoundTripNote label{display:flex;gap:8px;align-items:center;margin-top:8px;font-weight:800}
.dbestRoundBadge{display:inline-block;margin-left:6px;padding:3px 7px;border-radius:999px;background:#edf3ff;color:#245eea;font-size:8px;font-weight:900}
...
function tripDistance(){return F.route?Number(F.route.km||0)*(F.tripType==='roundtrip'?2:1):0}
function tripMinutes(){return F.route?Number(F.route.min||0)*(F.tripType==='roundtrip'?2:1):0}
function fare(v){if(!F.route||!Number(F.route.km))return null;if(F.tripType==='roundtrip'){const r=roundRules(),rule=r[v.id];if(!rule||!r.enabled)return null;return Math.round(tripDistance()*Number(rule.perKm||0)+(F.nightHalt?Number(rule.nightHalt||0):0))}const c=readRideConfig(),ov=(c?.vehicles||[]).find(x=>x.id===v.id);if(ov){let n=Math.max(Number(ov.minFare||0),Number(ov.base||0)+Number(ov.perKm||0)*F.route.km);n*=Number(c.surge||1);n+=Number(c.platformFee||0);n*=1+Number(c.taxPercent||0)/100;return Math.round(n)}return Math.round(Math.max(v.min,v.base+v.km*F.route.km))}
function veh(v){const x=liveVehicle(v),f=fare(x),r=roundRules(),rr=F.tripType==='roundtrip'?r[x.id]:null;return `<button type="button" class="cab6Veh ${F.selected===x.id?'on':''}" data-v="${x.id}"><span class="photo"><img src="${x.img}" alt="${esc(x.name)}" loading="lazy" decoding="async"></span><b>${esc(x.name)}${F.tripType==='roundtrip'?'<span class="dbestRoundBadge">ROUND TRIP</span>':''}</b><small>${x.seats} seat${x.seats>1?'s':''}</small><small class="dbestPickupEta">Pickup ~${x.eta} min</small><strong><span>${f===null?'₹…':'₹'+f}</span><em>${F.tripType==='roundtrip'?(rr?'₹'+rr.perKm+'/km • Tolls extra':'Round Trip'):(F.route?'Trip ~'+F.route.min+' min':'')}</em></strong></button>`}
function mapShell(title){return `<div class="cab6MapFrame"><div id="cab13Map" class="cab6Map"><div class="cab13MapFallback"><div><b>Preparing route map…</b><br><small>Ride options are available below</small></div></div></div></div>`}
...
function renderRideShell(){const ready=!!(F.route&&F.route.km),list=visibleVehicles();if(!list.some(v=>v.id===F.selected))F.selected=list[0]?.id||'sedan';const sel=liveVehicle(list.find(v=>v.id===F.selected)||list[0]);screen(`${mapShell(rideTitle())}<div class="cab6Sheet dbestSheetIn"><div class="dbestCabProgress"><span class="on"></span><span></span><span></span></div><div class="cab6SheetHead"><b>Choose a ride</b><small>${F.tripType==='roundtrip'?'Same vehicle • Return included • Tolls extra':'Upfront fare • Live Owner tariff'}</small></div><div class="cab6Meta"><div><small>${F.mode==='rental'?'Included distance':F.tripType==='roundtrip'?'Total round-trip distance':'Road distance'}</small><b id="cab13Km">${ready?(F.tripType==='roundtrip'?tripDistance():F.route.km).toFixed(1)+' km':'Calculating…'}</b></div><div><small>${F.mode==='rental'?'Rental duration':F.tripType==='roundtrip'?'Total driving time':'Estimated travel time'}</small><b id="cab13Min">${ready?(F.mode==='rental'?F.rentalHours+' hr':'~'+(F.tripType==='roundtrip'?tripMinutes():F.route.min)+' min'):'Calculating…'}</b></div></div><div id="cab13Status" class="cab13Status ${ready?(F.route.source==='Estimated route'?'warn':'ok')}">${esc(F.tripType==='roundtrip'?'Same vehicle reserved for outward and return journey • Tolls extra':statusText())}</div><div id="cab13Vehicles" class="cab6Vehicles">${list.map(veh).join('')}</div><button type="button" id="cab13Continue" class="cab6Book" ${ready?'':'disabled'}>${ready?'Continue with '+esc(sel?.name||'Ride'):'Calculating fare…'}</button></div>`,'DBEST_CAB_SELECTED_UI.open()');bindRideCards();if(ready)renderMap()}
function bindRideCards(){qa('.cab6Veh[data-v]').forEach(c=>c.onclick=()=>{F.selected=c.dataset.v;qa('.cab6Veh[data-v]').forEach(x=>x.classList.toggle('on',x===c));const b=$('cab13Continue'),v=visibleVehicles().find(x=>x.id===F.selected);if(b&&!b.disabled)b.textContent='Continue with '+(v?v.name:'Ride')});const b=$('cab13Continue');if(b)b.onclick=()=>renderConfirm()}
function updateRide(){if(!$('cab13Vehicles')||!F.route)return;const km=$('cab13Km'),mn=$('cab13Min'),st=$('cab13Status'),list=$('cab13Vehicles'),b=$('cab13Continue'),vs=visibleVehicles();if(!vs.some(v=>v.id===F.selected))F.selected=vs[0]?.id||'sedan';if(km)km.textContent=(F.tripType==='roundtrip'?tripDistance():F.route.km).toFixed(1)+' km';if(mn)mn.textContent=F.mode==='rental'?F.rentalHours+' hr':'~'+(F.tripType==='roundtrip'?tripMinutes():F.route.min)+' min';if(st){st.textContent=F.tripType==='roundtrip'?'Same vehicle reserved for outward and return journey • Tolls extra':statusText();st.className='cab13Status '+(F.route.source==='Estimated route'?'warn':'ok')}if(list){list.innerHTML=vs.map(veh).join('');bindRideCards()}if(b){b.disabled=false;b.textContent='Continue with '+(vs.find(v=>v.id===F.selected)?.name||'Ride');b.onclick=()=>renderConfirm()}renderMap()}
...
function renderConfirm(){
 ...
 <div class="cab6Fare"><div><small style="color:#858da0;font-size:9px">${F.tripType==='roundtrip'?'Round Trip estimated fare':'Upfront estimated fare'}</small><strong>₹${amount}</strong></div><div style="text-align:right;color:#7c8498;font-size:8px">${(F.tripType==='roundtrip'?tripDistance():F.route.km).toFixed(1)} km<br>${F.mode==='rental'?F.rentalHours+' hr':F.tripType==='roundtrip'?'Same vehicle • Tolls extra':'Trip ~'+F.route.min+' min'}</div></div>${F.tripType==='roundtrip'?'<div class="cab13Status ok" style="margin-top:8px">Return to pickup included • Same vehicle/Vaahak retained • Tolls extra'+(F.nightHalt?' • Night halt included':'')+'</div>':''}
...
function patchSearch(){css();
 ...
 if(drop&&!$('dbestTripType')){
   const rt=document.createElement('div'),rr=roundRules();rt.id='dbestTripType';rt.innerHTML='<div class="dbestTripType"><button type="button" class="on" data-dbest-trip="oneway">One Way</button><button type="button" data-dbest-trip="roundtrip">Round Trip</button></div><div id="dbestRoundTripNote"><b>Same vehicle round trip</b><br>Sedan ₹'+rr.sedan.perKm+'/km • SUV ₹'+rr.suv.perKm+'/km • Tolls extra<br><label><input type="checkbox" id="dbestNightHalt"> Night halt (Sedan ₹'+Number(rr.sedan.nightHalt).toLocaleString('en-IN')+' / SUV ₹'+Number(rr.suv.nightHalt).toLocaleString('en-IN')+')</label></div>';
   drop.insertAdjacentElement('afterend',rt);
   rt.querySelectorAll('[data-dbest-trip]').forEach(btn=>btn.onclick=()=>{rt.querySelectorAll('[data-dbest-trip]').forEach(x=>x.classList.toggle('on',x===btn));const round=btn.dataset.dbestTrip==='roundtrip';$('dbestRoundTripNote')?.classList.toggle('show',round);if(round)F.selected='sedan';try{window.DBEST_I18N?.apply?.();window.DBEST_USER_I18N?.apply?.()}catch(e){}});
 }
 if(drop&&!$('cab6AddStop')){
