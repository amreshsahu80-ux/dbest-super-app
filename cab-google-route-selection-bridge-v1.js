...
function installConfigProxy(){
  ...
  try{window.DBEST_RUNTIME_CONFIG=new Proxy(current,{get(target,prop,receiver){if(prop==='__dbestGooglePrimaryProxy')return true;if(prop==='mapplsStaticKey'&&googleReady()&&selectedCabCaller())return '';return Reflect.get(target,prop,receiver)}})}catch(e){console.warn('DBest Google primary config bridge warning',e)}
}
...
function ensureRouteRenderer(){if(!routeState.map)return null;if(!routeState.renderer){routeState.renderer=new google.maps.DirectionsRenderer({map:routeState.map,preserveViewport:false,suppressMarkers:false,polylineOptions:{strokeColor:'#00bde8',strokeWeight:7,strokeOpacity:1}});window.DBEST_CAB_GOOGLE_DIRECTIONS_RENDERER=routeState.renderer}return routeState.renderer}
async function updateRouteGps(){const state=routeState;if(!state.map||lastDirections)return;const pos=await gpsPosition(6000);if(!pos||state!==routeState||!state.map||lastDirections)return;try{state.map.setCenter(pos);state.map.setZoom(14);if(state.marker)state.marker.setPosition(pos);else state.marker=new google.maps.Marker({map:state.map,position:pos})}catch(e){}}
function paintRouteMap(){
  if(!googleReady())return;const el=document.getElementById('cab13Map');if(!el)return;
  try{
    if(routeState.el!==el||!routeState.map){routeState={el,map:null,renderer:null,marker:null,gpsRequested:false,appliedRevision:-1};el.dataset.dbestGoogleMap='1';el.dataset.dbestGoogleFallback='';el.innerHTML='';routeState.map=new google.maps.Map(el,{center:{lat:22.5937,lng:78.9629},zoom:5,streetViewControl:false,mapTypeControl:false,fullscreenControl:false,gestureHandling:'greedy'});window.DBEST_CAB_GOOGLE_ROUTE_MAP=routeState.map;ensureRouteRenderer()}else{triggerResize(routeState.map);ensureRouteRenderer()}
    if(lastDirections&&routeState.appliedRevision!==directionsRevision){try{routeState.marker?.setMap(null)}catch(e){}routeState.marker=null;routeState.renderer.setDirections(lastDirections);routeState.appliedRevision=directionsRevision}else if(!lastDirections&&!routeState.gpsRequested){routeState.gpsRequested=true;updateRouteGps()}
...
function routeSucceeded(res){if(!res?.routes?.[0])return;lastDirections=res;directionsRevision++;setTimeout(paintRouteMap,60);setTimeout(paintRouteMap,320);setTimeout(paintRouteMap,850)}
function patchDirections(){
  ...
  proto.route=function(request,callback){routeStarting(request);if(typeof callback==='function')return original.call(this,request,(res,status)=>{try{if(status==='OK')routeSucceeded(res)}catch(e){}return callback(res,status)});const out=original.call(this,request);if(out&&typeof out.then==='function')return out.then(res=>{try{routeSucceeded(res)}catch(e){}return res});return out};patchedDirections=true;
}
