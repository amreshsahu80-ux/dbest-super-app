...
/* Clean first-step cab screen without hiding booking fields */
.dcx .dcxHero,.dcx .dcxHint,.dcx .dcxBtns,.dcx .dcxStats,.dcx .dcxPowered{display:none!important}
.dcx .dcxPanel{display:block!important;padding:14px!important;margin:-18px 10px 0!important;border-radius:22px!important}
.dcx .dcxField,.dcx .dcxInput,.dcx .dcxActions{display:block!important}
.dcx .dcxField{margin-bottom:10px!important}
.dcx .dcxInput{display:grid!important}
.dcx .dcxActions{margin-top:12px!important}
...
.dbestCabFirstRider{display:block!important;margin:10px 0 12px;padding:11px;border:1px solid #dfe7f4;border-radius:16px;background:#f8faff}
...
.dbestCabFirstRiderFields.show{display:grid!important}
...
function hideUtilityChipsSafely(){const labels=['gps on','easy pin','smart search'];qa('.dcx button').forEach(b=>{const t=(b.textContent||'').trim().replace(/\s+/g,' ').toLowerCase();if(labels.some(x=>t===x||t.endsWith(x)))b.style.display='none'});const groups=qa('.dcx .dcxBtns');groups.forEach(g=>g.style.display='none')}
function restoreCorePanel(){const panel=q('.dcx .dcxPanel');if(panel)panel.style.removeProperty('display');qa('.dcx .dcxField,.dcx .dcxInput,.dcx .dcxActions').forEach(el=>el.style.removeProperty('display'))}
...
function mount(){css();restoreCorePanel();hideUtilityChipsSafely();addRiderChoice();removeDuplicateContinue();syncConfirm()}
