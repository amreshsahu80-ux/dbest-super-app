...
/* Keep the first cab step deliberately minimal. These selectors are global because older cab bridges can render outside .dcx. */
.dcxHero,.dcxHint,.dcxBtns,.dcxStats,.dcxPowered,.cabxNote,.c9Hint,.mcHint{display:none!important}
...
function cleanupLegacyControls(){
  /* Hide utility buttons individually so we never hide Pickup/Drop containers again. */
  const utility=[/^(📍\s*)?gps\s*on$/i,/^(🗺️\s*)?easy\s*pin$/i,/^(🔎\s*)?smart\s*search$/i,/^(📍\s*)?pickup\s*map$/i,/^(🏁\s*)?drop\s*map$/i,/^[•·]?\s*current$/i];
  qa('button').forEach(b=>{const t=(b.textContent||'').trim().replace(/\s+/g,' ');if(utility.some(r=>r.test(t)))b.style.setProperty('display','none','important')});
  qa('.dcxBtns').forEach(x=>x.style.setProperty('display','none','important'));
  qa('small,p,div').forEach(el=>{if(el.children.length>1)return;const t=(el.textContent||'').trim().replace(/\s+/g,' ');if(/^Next:\s*choose vehicle/i.test(t))el.style.setProperty('display','none','important')});
  const continues=qa('button').filter(b=>/continue\s*.*choose vehicle/i.test((b.textContent||'').trim()));
  continues.forEach((b,i)=>{if(i>0)b.style.setProperty('display','none','important')});
...
const mo=new MutationObserver(()=>setTimeout(mount,20));mo.observe(document.documentElement,{childList:true,subtree:true});
