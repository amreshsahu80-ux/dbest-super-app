...
function hideSpinner(){
  const x=document.getElementById('dbestSpinnerLayer');
  if(x){x.style.display='none';x.style.pointerEvents='none';}
}
...
function forceMerchantConfig(){
  try{
    if(typeof paymentSettings!=='undefined'&&paymentSettings){
      ...
      paymentSettings.upiId=VPA;
      paymentSettings.payeeName=DISPLAY_PAYEE;
      paymentSettings.mobile='';
      paymentSettings.instructions='Pay using the DBest ICICI Merchant UPI. After payment, enter the UTR / UPI reference for verification.';
...
window.dbestStartMembershipUPI=function(tier,amount,name){
  ...
  if(window.DBEST_ICICI_MERCHANT_QR?.launch)return window.DBEST_ICICI_MERCHANT_QR.launch(amt,'DBest Membership','MEM-'+String(amt));
...
window.DBEST_DIRECT_UPI={
  version:VERSION,
  upiId:VPA,
  buildUri:(amount,note)=>merchantUri(amount,note),
  pay:function(txId){
    const x=txById(txId);if(!x)return false;
    const a=amountFor(x);if(!(a>0))return false;
    try{sessionStorage.setItem('dbest_pending_upi_tx',JSON.stringify({txId:x.id,amount:a,at:Date.now()}))}catch(e){}
    if(window.DBEST_ICICI_MERCHANT_QR?.launch){window.DBEST_ICICI_MERCHANT_QR.launch(a,'DBest '+String(x.sub||x.section||'Payment'),x.id);return true;}
...
function hidePayU(root=document){
  root.querySelectorAll('.dbestDualPayCard').forEach(card=>{
    card.querySelectorAll('button').forEach(b=>{
      const s=String(b.getAttribute('onclick')||'')+' '+String(b.textContent||'');
      if(/openPayUPaymentChoice|PayU/i.test(s))b.style.display='none';
      if(/openICICIDirectPayment/i.test(s)){b.textContent='📲 Pay via UPI';b.style.display='block';b.style.width='100%';}
    });
  });
}
...
window.openICICIDirectPayment=function(txId){
  ...
  if(!tx||!host)return;
  const amount=amountFor(tx);
  const href=merchantUri(amount,'DBest '+String(tx.section||'Payment'));
  host.innerHTML=`<div class="payCard" style="border:1px solid #dbe6f8">
    ...
    <div class="notice" style="margin:10px 0">After successful payment, return here and enter the UTR / UPI reference for verification.</div>
    <form class="form" onsubmit="submitICICIDirectClaim(event,'${esc(tx.id)}')">
      <div class="f"><label>UTR / UPI Reference Number</label><input name="utr" minlength="6" maxlength="60" required placeholder="Enter payment reference"></div>
      <div class="f full"><button class="btn">Submit Payment for Verification</button></div>
    </form>
  </div>`;
};

function patchAlias(root=document){
  root.querySelectorAll('.registrationPage,.paymentPage,.dbestInternalUpiBox,.dbestDualPayCard').forEach(box=>{
    box.querySelectorAll('*').forEach(el=>{
      if(el.children.length===0&&String(el.textContent||'').includes(LONG_ALIAS))el.textContent=String(el.textContent).replaceAll(LONG_ALIAS,VPA);
    });
  });
}
let scheduled=false;
function run(){
  ...
  requestAnimationFrame(()=>{scheduled=false;forceMerchantConfig();hidePayU();patchAlias();try{window.DBEST_ICICI_MERCHANT_QR?.wireRegistration?.()}catch(e){}});
}
