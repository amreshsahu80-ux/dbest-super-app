...
function payuConfig(){
  const key=safe(process.env.PAYU_KEY||process.env.PAYU_MERCHANT_KEY,120);
  const salt=safe(process.env.PAYU_SALT||process.env.PAYU_MERCHANT_SALT,200);
  const mode=safe(process.env.PAYU_MODE||'test',20).toLowerCase()==='live'?'live':'test';
  return {key,salt,mode,configured:!!(key&&salt),paymentUrl:mode==='live'?'https://secure.payu.in/_payment':'https://test.payu.in/_payment',verifyUrl:mode==='live'?'https://info.payu.in/merchant/postservice.php?form=2':'https://test.payu.in/merchant/postservice.php?form=2'};
}
