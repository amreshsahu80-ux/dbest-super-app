...
async function persist(x){
  if(!x||!isMember()||String(x.userId||'')!==sid())return;
  try{typeof save==='function'&&save()}catch(e){}
  try{await window.DBEST_TRANSACTION_LEDGER?.record?.(x)}catch(e){}
  try{setTimeout(()=>window.DBEST_TRANSACTION_LEDGER?.syncAll?.(false),80)}catch(e){}
}
function noteNative(x){
  if(!x)return x;
  lastCreatedAt=Date.now();
  try{recent.set(fingerprint(x.section,x.sub,x.source||x.meta?.source||'DBest',x.partnerUrl||x.meta?.partnerUrl||x.details||''),lastCreatedAt)}catch(e){}
  persist(x);
  return x;
}
function installAddTxObserver(){
  const fn=window.addTx;
  if(typeof fn!=='function'||fn.__dbestUniversalPersist)return false;
  const wrapped=function(){const x=fn.apply(this,arguments);return noteNative(x)};
  wrapped.__dbestUniversalPersist=true;
  wrapped.__dbestLedgerWrapped=fn.__dbestLedgerWrapped||false;
  window.addTx=wrapped;
  return true;
}
function createTx({section,sub,amount=0,status='Initiated',partner='',source='DBest',details='',url='',meta={}}){
  ...
  let x=null;
  try{if(typeof addTx==='function')x=addTx(sid(),clean(section)||'DBest Service',clean(sub)||'Service Transaction',Number(amount||0),status,clean(partner),{details:clean(details),source,partnerUrl:url||'',...meta})}catch(e){console.warn('DBest transaction capture addTx',e)}
  ...
  if(!x){
    ...
    try{if(typeof txs!=='undefined'&&Array.isArray(txs))txs.unshift(x)}catch(e){}
  }
  lastCreatedAt=Date.now();
  persist(x);
  return x;
}
function safeExternal(raw){try{const u=new URL(String(raw||''),location.href);if(!/^https?:$/.test(u.protocol)||u.origin===location.origin)return null;return u}catch(e){return null}}
...
function installWindowOpen(){
  const fn=window.open;if(typeof fn!=='function'||fn.__dbestUniversalTx)return;
  const wrapped=function(url){try{recordExternal(url,document.activeElement,'External destination opened')}catch(e){}return fn.apply(this,arguments)};
...
document.addEventListener('click',function(e){
  ...
  const before=new Set(ownTxs().map(x=>String(x?.id||'')));
  setTimeout(()=>{if(newestAfter(before))return;const c=context(b);createTx({section:c.section,sub:c.sub,amount:amountFrom(b),status:'Initiated',source:'DBest Internal',details:label,meta:{fallbackCapture:true}})},350);
},true);
...
document.addEventListener('submit',function(e){
  ...
  const before=new Set(ownTxs().map(x=>String(x?.id||'')));
  setTimeout(()=>{if(newestAfter(before))return;const c=context(form);createTx({section:c.section,sub:c.sub,amount:amountFrom(form),status:'Submitted / Pending',source:'DBest Internal',details:'Service transaction submitted',meta:{fallbackCapture:true,form:true}})},400);
},true);

function installUniversalGuard(){
  const fn=window.dbestUniversalExternalGo;if(typeof fn!=='function'||fn.__dbestUniversalTxGuard)return false;
  const wrapped=function(id,i){
    const before=new Set(ownTxs().map(x=>String(x?.id||'')));const out=fn.apply(this,arguments);
    setTimeout(()=>{
...
async function reconcile(){if(!isMember())return;try{await window.DBEST_TRANSACTION_LEDGER?.syncAll?.(false)}catch(e){}}
