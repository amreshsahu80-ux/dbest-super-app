...
async function open(tier){try{const api=await core();api.open(String(tier||'').toLowerCase());const b=await bank();requestAnimationFrame(()=>{try{b?.scan?.()}catch(e){}});install()}catch(e){console.error('DBest registration',e);try{typeof toast==='function'?toast('Registration could not open. Please retry.'):alert('Registration could not open. Please retry.')}catch(_){}}}
async function ownerAdd(){try{const api=await core();api.ownerAdd();const b=await bank();requestAnimationFrame(()=>{try{b?.scan?.()}catch(e){}});install()}catch(e){console.error('DBest owner onboarding',e)}}
