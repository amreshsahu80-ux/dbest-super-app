...
async function ensureRegistration(v){
  if(!v?.id||!v?.email) throw new Error('Vendor registration details are incomplete.');
...
function friendly(err){
  ...
  if(k==='vendor_email_missing') return 'A valid vendor email is required before signing the agreement.';
...
function decorateScreen(id){
  setTimeout(()=>{
    const v=getVendor(id); if(!v) return;
    const sel=document.getElementById('vendorPartnerChannel');
    if(sel){sel.innerHTML=`<option value="email">Email ${esc(v.email||'')}</option>`;sel.value='email';sel.disabled=false}
    document.querySelectorAll('.demoOtp').forEach(x=>x.remove());
    const input=document.getElementById('partnerOtp_vendor_'+id);
    const state=getState(id);
    const firstSign=sel?.closest?.('.signCard')||document.querySelector('.signGrid .signCard');
...
window.sendPartnerOtp=async function(kind,id,who,channel){
  ...
  const v=getVendor(id); if(!v) return toast2('Vendor registration not found.');
  ...
  try{
    ...
    toast2('OTP sent to the registered vendor email.');
...
window.DBEST_VENDOR_AGREEMENT_EMAIL_OTP={version:VERSION,send:window.sendPartnerOtp,verify:window.verifyPartnerOtp};
})();