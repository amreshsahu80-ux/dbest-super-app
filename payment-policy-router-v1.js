(()=>{
'use strict';
const VERSION='1.0-dbest-payment-policy';
const POLICY={
  registrations:{user:['razorpay'],vaahak:['razorpay'],service_partner:['razorpay'],vendor:['razorpay']},
  ecommerce:['razorpay','payu'],cab:['cash_to_driver','upi_to_driver'],home_jobs:['razorpay','payu'],repairing:['razorpay','payu'],forms:['payu']
};
...
function restorePayU(){
  ...
  try{if(typeof payuSettings!=='undefined'&&payuSettings){payuSettings.createEndpoint='/api/payu/create-payment';payuSettings.buttonLabel='Pay Securely with PayU';}}
  catch{}
  try{if(typeof rideConfig!=='undefined'&&rideConfig){rideConfig.payu=false;rideConfig.cash=true;rideConfig.upiDriver=true;}}
  catch{}
}
...
function classify(tx){
  ...
  if(/(home jobs?|job search|job application|hyperlocal jobs?|hiring)/.test(s))return 'home_jobs';
...
function gatewayHtml(tx,route){
  ...
  if(route==='ecommerce'||route==='home_jobs'||route==='repairing')return `<div class="ownerPanelCard dbestPolicyCard" data-dbest-policy="${route}"><h3>Choose Payment Gateway</h3>${head}<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">${razorButton(tx.id,true)}${payuButton(tx.id,true)}</div><small style="display:block;margin-top:8px">Razorpay is primary. PayU is the alternate gateway.</small></div>`;
...
function decorate(txId){
  ...
  const target=root.querySelector('.payCard')||root;target.insertBefore(card,target.firstChild);
  setTimeout(refreshPolicyCards,0);
}
