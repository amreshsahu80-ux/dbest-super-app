(function(){
'use strict';
const V='20260922-cab-experience-v2-entry-redesign';
if(window.DBEST_CAB_ENTRY_CAPTURE?.version===V)return;
const ASSET_V='20260922-cab-experience-v2-entry-redesign';
...
function optimizeImages(){
  try{document.querySelectorAll('#dbestSelectedCabModal img,#dbestCabSelectedModal img,.dbestCabSelected img,.cabSelected img,.cab6Page img').forEach(img=>{img.loading='lazy';img.decoding='async'})}catch(_){ }
}
async function openApproved(){
  ...
  }finally{setTimeout(()=>{opening=false},80)}
}
...
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',scheduleWarm,{once:true});else scheduleWarm();
