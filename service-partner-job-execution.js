...
function hideLegacyList(){
 document.querySelectorAll('.ownerPanelCard h3').forEach(h=>{if(/^Assigned Home Jobs$/i.test(text(h.textContent))&&h.closest('#dbestServiceJobExecution')==null){const p=h.closest('.ownerPanelCard');if(p)p.style.display='none'}});
}
async function render(){
 if(!isPortal()||!hasSession())return false;
 ...
 try{
   ...
   hideLegacyList();return true;
 }catch(e){
   if(['owner_approval_required','service_partner_not_registered'].includes(e.message))return false;
   box.innerHTML='<h3>🧰 Live Job Execution</h3><div class="notice">Live jobs could not be loaded. Please refresh or login again.</div>';return false;
 }
}
...
document.addEventListener('click',()=>setTimeout(render,450),true);
