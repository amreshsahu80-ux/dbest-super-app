(function(){
  ...
  function isOwnerScreen(root){
    try{
      if(window.session && ['owner','admin'].includes(String(window.session.role||'').toLowerCase())) return true;
      if(root?.classList?.contains('owner55') || root?.classList?.contains('ownerStudio')) return true;
      const top=(document.querySelector('.sectionTopBar,.sectionHeader,.topbar')?.innerText||'').toLowerCase();
      const body=(root?.innerText||'').slice(0,700).toLowerCase();
      return /owner operations|project owner|master control|payout rules|tile media studio|external deeplinks/.test(top+' '+body);
  ...
  function apply(){
    ...
    if(!root || isOwnerScreen(root)) return;
    const kind=detectKind(root); if(!kind)return;
    ...
    root.querySelectorAll('form').forEach(f=>{ if(!deeplink.contains(f)) f.remove(); });
    ...
    if(!root.querySelector(`[data-dbest-showcase="${kind}"]`)){
      ...
      deeplink.insertAdjacentElement('afterend',show);
    }
  }
  ...
  let t=null; const schedule=()=>{clearTimeout(t);t=setTimeout(apply,60)};
